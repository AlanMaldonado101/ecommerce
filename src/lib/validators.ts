...
export const userRegisterSchema = z.object({
	...
	password: z
		...
		.regex(/[A-Z]/, 'La contraseña debe incluir al menos 1 mayúscula')
		.regex(
			/[^A-Za-z0-9]/,
...
export const productSchema = z.object({
	...
	occasion_ids: z.array(z.string()).optional(),
	// Las características se mantienen a nivel de tipo pero ya no son obligatorias en el formulario
	features: z
		.array(
			z.object({
				value: z
	...
	variants: z
		.array(
			z.object({
				...
				stock: z.number(),
				price: z.number().min(0.01, 'El precio por menor debe ser mayor a 0'),
				priceWholesale: z.number().min(0, 'El precio por mayor no puede ser negativo'),
				storage: z.string().min(1, 'La presentación es requerida'),
				color: z
					.string()
					.regex(
